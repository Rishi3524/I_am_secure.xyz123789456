                            ...
                            login_session_data_t *session)
{
    if (!userid || !password || !session) {
        ...
        dprintf(log_fd, "ERROR: handle_login: NULL input\n");
    ...
    if (!account_validate_password(&acc, password)) {
        ...
        dprintf(log_fd, "WARNING: incorrect password for user '%s'\n", userid);
